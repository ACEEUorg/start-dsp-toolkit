...
- **Asset fallbacks are runtime, not build-time.** `src/utils/pdfFallback.js` does a `HEAD` request per PDF and rewrites `/tools/{lang}/` → `/tools/en/` when the localized file is missing (it also rejects `text/html` responses, since the dev server answers those for missing files); `ToolImage.jsx` walks `.jpg → .jpeg → .png` before falling back to a placeholder. So a missing translated PDF or image is a normal, handled state — don't "fix" it by editing frontmatter URLs.
